...
using { OrdersService2 } from '../srv/orders-service2';
...
annotate OrdersService.Items with @(
	UI: {
		LineItem: [
			{Value: category.description, Label:'{i18n>Category}'},
...
//// Annotations for Order Service 2

@odata.draft.enabled
annotate OrdersService2.Orders with @(
	UI: {
		SelectionFields: [ createdBy ],
		LineItem: [
			{Value: OrderNo, Label:'{i18n>OrderNo}'},
...
annotate OrdersService2.Items with @(
	UI: {
		LineItem: [
			{Value: category.description, Label:'{i18n>Category}'},
			{Value: product_ID, Label:'{i18n>ProductID}'},
